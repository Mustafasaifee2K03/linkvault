...
export default function View() {
  ...
  if (data.requiresPassword && !verified) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center px-4 text-red-200">
        <div className="bg-black/70 border border-red-900/50 rounded-xl p-8 shadow-[0_0_40px_rgba(255,0,0,.4)]">
          ...
          <input
            ...
            className="w-full bg-black border border-red-900/50 rounded p-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-600"
            value={password}
            onChange={(e) => setPassword(e.target.value)}
          />
          <button
            onClick={verifyPassword}
            className="w-full mt-4 bg-red-700 hover:bg-red-800 py-2 rounded tracking-widest font-semibold"
          >
            BREAK SEAL
          </button>
        </div>
